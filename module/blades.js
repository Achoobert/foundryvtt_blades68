...
Hooks.once("init", async function () {
  ...
  game.system.blades68Keys = Object.keys(blades68DeadlockedKeys).map(
    (name) => ({
      id: name,
      label: `BITD.Key${name}`,
  ...
  Handlebars.registerHelper("is_base_turf", (name) => {
    return BASE_TURF_NAMES.includes(
      String(name ?? "")
        .trim()
        .toLowerCase(),
    );
  ...
  Handlebars.registerHelper(
    "repturf",
    (_id, turfs_amount, max_rep, options) => {
      let html = options.fn(this);
      var turfs_amount_int = parseInt(turfs_amount) || 0;
      for (let i = 1; i <= max_rep; i++) {
        const claimedClass =
          i > max_rep - turfs_amount_int ? " turf-claimed" : "";
  ...
  Handlebars.registerHelper(
    "blades-clock-color",
    function (parameter_name, type, color, current_value, uniq_id) {
      let html = "";
...
Hooks.once("ready", async function () {
  ...
  registerActorSheet("blades", BladesActorSheet, {
    types: ["character"],
    makeDefault: true,
  });
  registerActorSheet("blades", BladesCrewSheet, {
    types: ["crew"],
    makeDefault: true,
  });
  registerActorSheet("blades", BladesFactionSheet, {
    types: ["factions"],
    makeDefault: true,
  });
  registerActorSheet("blades", BladesClockSheet, {
    types: ["\uD83D\uDD5B clock"],
    makeDefault: true,
  });
  registerActorSheet("blades", BladesNPCSheet, {
    types: ["npc"],
    makeDefault: true,
  });
...
for (const hook of [
  ...
  Hooks.on(hook, (effect) =>
    rerenderLinkedCharacterSheets(ownerActorOf(effect)),
  );
}
...
const PAUSE_IMAGE =
  "systems/blades68/styles/assets/blades68/bladesin68_logo.webp";
const PAUSE_IMAGE_CLASSIC = "systems/blades68/images/BITD_logo.webp";
...
const PAUSE_MODES = ["vhs", "bluetime", "vanilla", "classic"];
...
function setPauseImage(target) {
  ...
  if (image) {
    image.src = mode === "classic" ? PAUSE_IMAGE_CLASSIC : PAUSE_IMAGE;
  }
  if (mode === "classic") {
  ...
  // "vanilla" and "classic" are bare logos: no background layers, nothing running
  if (mode === "vanilla" || mode === "classic") return;
