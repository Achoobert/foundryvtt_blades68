...
export default class FactionTrackerApp extends HbsAppMixin(ApplicationV2) {
  ...
  async _prepareContext(options) {
    ...
    const factions = game.actors.filter((actor) => actor.type === 'faction');

    context.config = BLADES68;
    context.columns = BLADES68.FACTION_CATEGORIES.map((category) => ({
      category,
      factions: factions
        .filter((actor) => actor.system.category === category)
  ...
  static async _onSetTier(event, target) {
    ...
    const value = actor.system.tier === clicked ? clicked - 1 : clicked;
    await actor.update({ 'system.tier': value });
  }

  static async _onCycleHold(event, target) {
    const actor = game.actors.get(target.dataset.actorId);
    const next = actor.system.hold === 'weak' ? 'strong' : 'weak';
    await actor.update({ 'system.hold': next });
  }

  static async _onAdjustStatus(event, target) {
    const actor = game.actors.get(target.dataset.actorId);
    const delta = Number(target.dataset.delta);
    const value = Math.min(3, Math.max(-3, actor.system.status + delta));
    await actor.update({ 'system.status': value });
  }

  static async _onToggleWar(event, target) {
    const actor = game.actors.get(target.dataset.actorId);
    await actor.update({ 'system.war': !actor.system.war });
  }
